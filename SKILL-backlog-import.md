...
Then print this vetting prompt — exactly as written:

> **Review each story above before pushing.** Request any edits, rewrites, or additions now. When every story is complete and accurate, reply **"approved — push to [PROJECT KEY]"** to send them to Jira.

Do not proceed to Step 6 until the user sends an explicit approval message. Treat any response other than a clear approval as a request to revise. Apply requested edits and re-output the affected stories before asking again.

---

### Step 6 — Jira Push (gated on explicit approval)

Only execute after the user has explicitly approved the stories as complete and vetted. The project key is taken from their approval message (e.g., "approved — push to ICLD").

**Never push unreviewed stories.** If the user asks to push without having reviewed the output in this session, restate the vetting requirement and display the stories first.

Use the `mcp-atlassian` MCP server to create issues. **For each story, call `jira_create_issue` with:**

| Jira field | Source |
|---|---|
| `project_key` | From user's approval message (e.g., `ICLD`) |
| `summary` | `{title}` |
